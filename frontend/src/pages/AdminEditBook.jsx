...
export const AdminEditBook = ({ UserRole }) => {
  ...
  const stateOptions = ["Gujarat", "Rajasthan", "Maharashtra"];
  ...
  const handleSubmit = async (e) => {
    ...
    // Additional validation for new fields based on subcategory name
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_BACK_URL}/api/${UserRole}/Book`,
          {
            method: "POST",
            body: formDataToSend,
            credentials: "include",
        ...
        if (json.book) {
          ...
          showAlert(json.message || "Book not updated", "error");
        }
      } catch (error) {
