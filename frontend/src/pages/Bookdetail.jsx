...
export const Bookdetail = () => {
  ...
  const fetchAverageRating = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACK_URL}/api/Ratings/average/${book._id}`, {
        headers: {
  ...
  const addRatings = async (currentRating) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACK_URL}/api/Ratings`, {
        method: "POST",
        headers: {
          authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
