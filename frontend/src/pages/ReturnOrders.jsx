...
const ReturnOrders = () => {
  ...
  useEffect(() => {
    const fetchReturnOrders = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_BACK_URL}/api/returnorder`, {
          credentials: "include",
          headers: {
  ...
  const handleStatusUpdate = async (id, status, orderId) => {
    try {
      ...
      // 2. Update the main order status based on return order status
      const orderStatus =
        status === "Approved"
          ? "return-pending"
          : status === "Rejected"
      ...
      // 3. Update the UI with the updated return order
      setReturnOrders((prevOrders) =>
        prevOrders.map((order) => (order._id === id ? updatedReturnOrder : order))
      );
                    ...
                    {order.status === "Pending" ? (
                      <>
                        <button
                          ...
                          onClick={() =>
                            handleStatusUpdate(order._id, "Approved", order.order_id)
                          }
                        >
                          Accept
                        </button>
                        <button
                          ...
                          onClick={() =>
                            handleStatusUpdate(order._id, "Rejected", order.order_id)
                          }
                        >
                          Reject
                        </button>
